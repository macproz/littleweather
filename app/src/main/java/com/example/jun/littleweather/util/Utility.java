...
public class Utility {
    ...
    public synchronized static boolean handleProvincesResponse(
            LitterWeatherDB litterWeatherDB, String response){
        if(!TextUtils.isEmpty(response)){
            ...
            if(allProvinces != null && allProvinces.length > 0){
                for(String p : allProvinces){
                    ...
                    litterWeatherDB.saveProvince(province);
                }
                return true;
            }
        }
    ...
    public  static boolean handleCitiesResponse(
            LitterWeatherDB litterWeatherDB, String response, int provinceId){
        if(!TextUtils.isEmpty(response)){
    ...
    public  static boolean handleCountriesResponse(
            LitterWeatherDB litterWeatherDB, String response, int cityId){
        if(!TextUtils.isEmpty(response)){
            ...
            if(allcountries != null && allcountries.length > 0){
                for(String c : allcountries){
                    ...
                    litterWeatherDB.saveCountry(county);
                }
                return true;
            }
        }
